...
export function QuestionNavigationPanel({
    ...
    if (quizState === 'reviewing') {
      if (isAnswered) {
        if (userAnswer === question.correctAnswer) {
         ...
         icon = <HelpCircle className="h-4 w-4 opacity-70" />;
         srText += ", Not Answered";
      }
      if (isCurrent) {
         statusStyle = `${statusStyle} ring-2 ring-primary ring-offset-2 dark:ring-offset-background shadow-lg`;
         srText += ", Currently Viewing";
      }
    } else { // Taking quiz
      if (isCurrent) {
        statusStyle = "bg-primary text-primary-foreground hover:bg-primary/90 ring-2 ring-primary/50 ring-offset-2 dark:ring-offset-background shadow-lg";
        srText += ", Currently Viewing";
      } else if (isMarked) {
  ...
  const legendItems = quizState === 'reviewing' ? [
    ...
    { label: "Marked", icon: <Eye className="h-3.5 w-3.5 text-yellow-700 dark:text-yellow-300" />, style: "bg-yellow-400/30 border-yellow-500" },
            ...
            {Array.from({ length: totalQuestions }).map((_, index) => {
              ...
              return (
                <Button
                  ...
                  variant="outline"
                  className={cn(
                    "h-11 w-11 sm:h-12 sm:w-12 p-0 relative flex items-center justify-center text-md font-medium rounded-lg aspect-square transition-all",
                    statusStyle
                  )}
                ...
                >
                  ...
                  {quizState === 'taking' && markedForReview[index] && index !== currentQuestionIndex && (
                     <Eye className="absolute top-1 right-1 h-4 w-4 text-yellow-600 dark:text-yellow-400 bg-card p-0.5 rounded-full shadow-md" />
                  )}
                </Button>
              );
            })}
